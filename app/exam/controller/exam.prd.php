...
class action extends prd
{
    ...
    private function reload()
    {
        ...
        header("location:index.php?exam-prd-exam");
    }
    ...
    private function makescore()
    {
        ...
        if ($this->ev->get('makescore')) {
            ...
            if ($this->ev->get('direct')) {
                if ($id)
                    header("location:index.php?exam-prd-exam-makescore&ehid={$id}"); else
                    header("location:index.php?exam-prd-exam-paper");
                exit;
            } else {
                if ($id)
                    $message = array('statusCode' => 200, "message" => "操作成功", "callbackType" => 'forward', "forwardUrl" => "index.php?exam-prd-exam-makescore&ehid={$id}"); else
    ...
    private function score()
    {
        ...
        if ($this->ev->get('insertscore')) {
            ...
            foreach ((array)$question as $key => $a)
            ...
            if (!$needhand) {
                ...
                $this->exam->modifyExamSession($args);
                $message = array('statusCode' => 200, "message" => "操作成功", "callbackType" => 'forward', "forwardUrl" => "index.php?exam-prd-exam-makescore&makescore=1&direct=1");
            } else {
                if ($sessionvars['examsessionsetting']['examdecide']) {
                    ...
                    $message = array('statusCode' => 200, "message" => "操作成功", "callbackType" => 'forward', "forwardUrl" => "index.php?exam-prd-exam-score");
                }
            }
            $this->G->R($message);
        } else {
            if ($sessionvars['examsessionstatus'] == 2) {
                header("location:index.php?exam-prd-exam-makescore");
                exit;
            } else {
                $this->tpl->assign('sessionvars', $sessionvars);
                $this->tpl->assign('questype', $questype);
    ...
    private function paper()
    {
        ...
        if ($sessionvars['examsessionstatus'] == 2) {
            ...
            header("location:index.php?exam-prd-exam-score");
            exit;
        } else {
    ...
    private function selectquestions()
    {
        ...
        if (!$r['examid']) {
            ...
            if ($r['examtype'] == 1) {
                ...
                $message = array('statusCode' => 200, "message" => "抽题完毕，转入试卷页面", "callbackType" => 'forward', "forwardUrl" => "index.php?exam-prd-exam-paper");
                $this->G->R($message);
            } elseif ($r['examtype'] == 2) {
                $sessionvars = $this->exam->getExamSessionBySessionid();
                $questions = array();
